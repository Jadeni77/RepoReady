...
describe("planFix", () => {
    ...
    it("proposes a fix for every failing check that has a generator", async () => {
        const plan = await planFix({ cwd: await temp({ "package.json": "{}" }) });
    ...
    it("reports the score before any change", async () => {
        ...
        const plan = await planFix({ cwd: root });
    ...
    it("marks a fix risky when it would overwrite an existing file", async () => {
        ...
        const plan = await planFix({ cwd: await temp({ "CODE_OF_CONDUCT.md": "" }) });
...
describe("runFix", () => {
    ...
    describe("--dry-run", () => {
        it("writes nothing", async () => {
            const root = await temp({ "package.json": "{}" });
            const result = await runFix({ cwd: root, dryRun: true });
    ...
    describe("--yes", () => {
        ...
        it("raises the doctor score", async () => {
            ...
            const before = await runDoctor({ cwd: root });
            await runFix({ cwd: root, yes: true });
            const after = await runDoctor({ cwd: root });
        ...
        it("is idempotent — a second run has nothing left to do", async () => {
            ...
            await runFix({ cwd: root, yes: true });
            const second = await runFix({ cwd: root, yes: true });
    ...
    describe("safety", () => {
        ...
        it("applies only the selected fixes", async () => {
            const root = await temp({ "package.json": "{}" });
            const result = await runFix({ cwd: root, yes: true, select: ["license"] });
        ...
        it("accepts a check ID as a selector", async () => {
            const root = await temp({ "package.json": "{}" });
            await runFix({ cwd: root, yes: true, select: ["pr-template"] });
    ...
    it("passes the license and author through to the generator", async () => {
        const root = await temp({ "package.json": "{}" });
        await runFix({ cwd: root, yes: true, license: "isc", author: "Ada Lovelace" });
    ...
    it("passes the CI language through to the generator", async () => {
        const root = await temp({ "package.json": "{}" });
        await runFix({ cwd: root, yes: true, lang: "go" });
    ...
    it("reports nothing to do for an already healthy repo", async () => {
        ...
        const again = await runFix({ cwd: root, yes: true });
